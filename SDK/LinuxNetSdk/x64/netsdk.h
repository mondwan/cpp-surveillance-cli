...
typedef enum _SDK_CONFIG_TYPE
{
	...
	E_SDK_CONFIG_CHANNELTILE_DOT,		//点阵信息-修改IPC通道名需要点阵信息
	...
	E_SDK_SET_OSDINFO,				//OSD信息设置---对应结构体SDK_OSDInfo
	E_SDK_SET_OSDINFO_V2,			//OSD信息叠加，不保存配置---对应结构体SDK_OSDInfoConfigAll
	...
	E_SDK_CFG_CUSTOM_ENVIRONMENT,   //对应的结构体SDK_CustomEnvirData
}SDK_CONFIG_TYPE;
...
enum SDK_NetServerTypes
{
    ...
    SDK_NET_NET_SERVER_TYPES_BJLTHY,				//北京龙腾
...
enum SDK_OtherFunction
{
    ...
    SDK_OTHER_SUPPORT_USE_PROGRAM,//支持启动客户小程序
...
typedef struct SDK_SearchByTime
{
    ...
    int    iSync;               ///< 是否需要同步
    unsigned int	nHighStreamType;///< 33~64录像的码流类型
    unsigned int nLowStreamType;///< 1~32录像的码流类型
}SDK_SearchByTime;
...
typedef struct  SDK_OSDInfoConfigAll
{
    ...
    int strEnc;//编码格式,默认为UTF-8,不做任何处理；主要用于客户特殊需求，枚举参见wordEncType
...
//////////////////////////////////////////////////////////////////////////
typedef struct SDK_CustomEnvirData
{
	bool bShow;
	char pName[48];
	char pUnit[24];
	int   lowerValue;
	int   upperValue;
	int   alarmOutType; //0: 上限报警   1:下限报警   2:无
	char resv[8];
}SDK_CustomEnvirData;

typedef struct SDK_CustomEnvirDataAll
{
	SDK_CustomEnvirData envirData[6];
	sdkRect showRect;
}SDK_CustomEnvirDataAll;

...
BOOL CDlgChannelMode::OnInitDialog() 
{
	...
	m_ctrlList.InsertColumn(5,_CS(_T("HD1")),LVCFMT_LEFT,50);
	
	m_ctrlList.InsertColumn(6, _CS(_T("")),LVCFMT_LEFT,13);
	
	m_ctrlList.InsertColumn(7, _CS(_T("1080P")), LVCFMT_LEFT, 50);
	m_ctrlList.InsertColumn(8, _CS(_T("720P")), LVCFMT_LEFT, 50);
	m_ctrlList.InsertColumn(9, _CS(_T("960H")), LVCFMT_LEFT, 50);
	m_ctrlList.InsertColumn(10, _CS(_T("D1")),LVCFMT_LEFT,50);
	m_ctrlList.InsertColumn(11,_CS(_T("HD1")),LVCFMT_LEFT,50);
...
bool CDlgChannelMode::StructToWinData()
{
	...
	for ( int i = 0 ;  i < m_ChnModeCfg.HVRTotalCap.nHVRCap; i ++)
	{
		...
		m_ctrlList.SetItemText(i, j ++, strTemp);
		strTemp.Format(_T("%d"), m_ChnModeCfg.HVRTotalCap.HVRCap[i].AnalogCap.nHD1Chn);
		m_ctrlList.SetItemText(i, j++,strTemp);
		...
		m_ctrlList.SetItemText(i, ++ j, strTemp);
		strTemp.Format(_T("%d"), m_ChnModeCfg.HVRTotalCap.HVRCap[i].DigitalCap.nHD1Chn);
		m_ctrlList.SetItemText(i, ++ j, strTemp);
	}

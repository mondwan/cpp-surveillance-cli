...
BOOL CAlarmMotion::SaveMotionInfo()
{
	...
	if (nIndex == m_nChannelNum)
	{
		for (int i = 0;i<m_nChannelNum;i++)
		{
			...
			m_MotionCfgAll.vMotionDetectAll[i].hEvent.bLog = m_ctlLogInfo.GetCheck();
	...
	}else
	{
		...
		m_MotionCfgAll.vMotionDetectAll[m_nLastChannel].hEvent.bLog = m_ctlLogInfo.GetCheck();
...
void CAlarmMotion::ShowInfo(int nIndex)
{
	...
	m_ctlLogInfo.SetCheck(m_MotionCfgAll.vMotionDetectAll[nIndex].hEvent.bLog);

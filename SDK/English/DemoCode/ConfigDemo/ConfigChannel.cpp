...
// };
static int STREAM_CODE_VALUE[SDK_CAPTURE_SIZE_EXT_V3_NR][6] = {
	{512, 768, 1024, 1536, 2048, 2560},	///< D1
	{384, 512, 768, 1024, 1536, 2048},	///< HD1
	{384, 512, 768, 1024, 1536, 2048},	///< BCIF
	{64, 192, 384, 512, 768, 1024},		///< CIF
	...
	{1536, 3072, 4096, 6144, 8192, 10240}, ///< 3M
	{2048, 3072, 4096, 6144, 8192,10240}, ///5M
	{2048, 3072, 4096, 6144, 8192,10240} ///1080N
};
...
bool CConfigChannel::AllStructToWinData(int nChannelNum, SDK_CONFIG_ENCODE_SIMPLIIFY *pConfigEncode, int nEncodeType)
{
	if (nEncodeType == MAIN_STREAM || (nEncodeType == EXPAND_STREAM && m_bMultiStream))
	{
		...
		if (nEncodeType == MAIN_STREAM)
		{
			...
			//最大的公共分辨率
			int nTmp = pConfigEncode->dstMainFmt.vfFormat.iResolution;
			i = GetMaxResolution(uiResolution);
			//如果当前的分辨率大于最大的公共分辨率，取最大公共分辨率
			if (i >=0 && m_resolution[i].dwSize < m_resolution[nTmp].dwSize)
			{
				pConfigEncode->dstMainFmt.vfFormat.iResolution = i;
			}
		...
		else if (nEncodeType == EXPAND_STREAM)
		{
			...
			{
				//早期版本是没有单通道的能力级的
				if (m_pEncodeAbility.ExImageSizePerChannelEx[i][pConfigEncode->dstMainFmt.vfFormat.iResolution])
				{
					uiResolution &= m_pEncodeAbility.ExImageSizePerChannelEx[i][pConfigEncode->dstMainFmt.vfFormat.iResolution];
				}
			}
			
			//如果单通道有能力级，就按照单通道的能力级来计算,选择全时分辨率要是大家都有的
			if (m_pEncodeAbility.ExImageSizePerChannelEx[nChannelNum][pConfigEncode->dstMainFmt.vfFormat.iResolution])
			{
				dwMask = m_pEncodeAbility.ExImageSizePerChannelEx[nChannelNum][pConfigEncode->dstMainFmt.vfFormat.iResolution] & uiResolution;
			}
			else
			{
				dwMask = m_pEncodeAbility.vEncodeInfo[0].uiResolution;
			...
			i = GetMaxResolution(uiResolution);
			//如果当前的分辨率大于最大的公共分辨率，取最大公共分辨率
			if (i >= 0 && m_resolution[i].dwSize < m_resolution[pConfigEncode->dstExtraFmt.vfFormat.iResolution].dwSize)
			{
				//pConfigEncode->dstMainFmt[0].vfFormat.iResolution = i;
				pConfigEncode->dstExtraFmt.vfFormat.iResolution = i;  //辅码流分辨率
			}
		}
...
// 	}
	...
	long lMaxSize = nSize;
	for ( int i = 0; i < SDK_CAPTURE_SIZE_EXT_V3_NR; i ++ )
	{
...
bool CConfigChannel::NormalStructToWinData(int nChannelNum, SDK_CONFIG_ENCODE_SIMPLIIFY *pConfigEncode, int nEncodeType)
{
	if (nEncodeType == MAIN_STREAM || (nEncodeType == EXPAND_STREAM && m_bMultiStream))
	{
		...
		if (nEncodeType == MAIN_STREAM)
		{
			...
			int i = 0;
			//最大的公共分辨率
			i = GetMaxResolution(m_pEncodeAbility.ImageSizePerChannel[nChannelNum]);
			//如果当前的分辨率大于最大的公共分辨率，取最大公共分辨率
			if (i >=0 && m_resolution[i].dwSize < m_resolution[pConfigEncode->dstMainFmt.vfFormat.iResolution].dwSize)
			{
				pConfigEncode->dstMainFmt.vfFormat.iResolution = i;
			}
		...
		else if (nEncodeType == EXPAND_STREAM)
		{
			int i = 0;
			//最大的公共分辨率
			//i = GetMaxResolution(m_abilityEncode.ExImageSizePerChannel[nChannelNum]);
			i = GetMaxResolution(m_pEncodeAbility.ExImageSizePerChannelEx[nChannelNum][pConfigEncode->dstMainFmt.vfFormat.iResolution]);
			//如果当前的分辨率大于最大的公共分辨率，取最大公共分辨率
			if (i >=0 && m_resolution[i].dwSize < m_resolution[pConfigEncode->dstExtraFmt.vfFormat.iResolution].dwSize)
			{
				pConfigEncode->dstExtraFmt.vfFormat.iResolution = i;
			}
		}
...
{
	int nChannelNum =GetComboBoxData(IDC_COMBO_CHANNEL);//m_ChanNo.GetCurSel();
...
int CConfigChannel::GetMaxResolution(UINT uiResolution)
{
	int nTmpSize = 0, nIndex = -1;
	for (int i = 0; i < SDK_CAPTURE_SIZE_EXT_V3_NR; i++)
	{
		if ((1 << i) & uiResolution)
		{
			if (nTmpSize < m_resolution[i].dwSize)
			{
				nTmpSize = m_resolution[i].dwSize;
				nIndex = i;
			}
		}
	}

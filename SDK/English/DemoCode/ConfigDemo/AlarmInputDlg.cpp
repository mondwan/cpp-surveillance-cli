...
void CAlarmInputDlg::OnCheckEnablealarm() 
{
	...
	GetDlgItem(IDC_STATIC_LOGINFO)->EnableWindow(bRet); //日志
...
BOOL CAlarmInputDlg::SaveAlarmInputInfo()
{
	...
	if (nIndex == m_nAlarmInNum)
	{
		for (int i = 0 ;i < m_nAlarmInNum ;i++)
		{
			...
			m_AlarmInputCfgAll.vAlarmConfigAll[i].hEvent.bLog = m_ctlLogInfo.GetCheck();
	...
	else
	{
		...
		m_AlarmInputCfgAll.vAlarmConfigAll[m_nLastChannel].hEvent.bLog = m_ctlLogInfo.GetCheck();

...
BOOL CConfigNetServer::OnInitDialog() 
{
	...
	m_pmsDlg.Create(IDD_CONFIG_NETSERVER_PMS);
...
// 				break;
			...
			case SDK_NET_SERVER_TYPES_NAT:
				...
				break;
			case SDK_NET_SERVER_TYPES_PMS:
				{
					nRet = m_listNetServer.InsertItem(nIndex ++, _CS("ConfigNet.PMS"));
					m_listNetServer.SetItemData(nRet,i);
				}
				break;
			default:
...
void CConfigNetServer::OnDblclkListNetserver(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	case SDK_NET_SERVER_TYPES_WIFI:
		{
			...
			m_bChange[SDK_NET_SERVER_TYPES_WIFI]=true;
		}
		break;
	case SDK_NET_SERVER_TYPES_ALARM_CENTER:
		{
	...
	case SDK_NET_SERVER_TYPES_PMS:
		{
			pMainDlg->GetPMSConfigInfo();
			m_pmsDlg.ShowWindow(SW_SHOW);
			m_bChange[SDK_NET_SERVER_TYPES_PMS] = true;
		}
		break;
	default:
		break;
...
void CConfigNetServer::OnButtonApply() 
{
	...
	CString strMsg[] = {"ConfigNet.IPSetting", "ConfigNet.DHCP", "ConfigNet.DDNS", "ConfigNet.EmailConfig", "ConfigNet.MulticastConfig", 
		...
		"ConfigNet.NetInfo" , "ConfigNet.GodEye","ConfigNet.PMS"};
...
// 				}
			...
			case SDK_NET_SERVER_TYPES_NAT:
				...
				break;
			case SDK_NET_SERVER_TYPES_PMS:
				if (m_pmsDlg.m_ensure)
				{
					nRet[SDK_NET_SERVER_TYPES_PMS] = pMainDlg->SetPMSConfigInfo(&m_pmsDlg.m_netPMSCfg);
					if ( nRet[SDK_NET_SERVER_TYPES_PMS] == H264_DVR_OPT_REBOOT )
					{
						nRet[SDK_NET_SERVER_TYPES_PMS] = 1;
						bReboot = true;
					}
				}
				break;

...
BEGIN_MESSAGE_MAP(Cdlgcoverfile, CDialog)
	...
	ON_EN_CHANGE(IDC_EDIT_DesPath, &Cdlgcoverfile::OnEnChangeEditDespath)
END_MESSAGE_MAP()
...
void Cdlgcoverfile::OnOK() 
{
	...
	if(m_OldDes == m_strDesPath.Mid(m_strDesPath.ReverseFind('\\')+1))
	{
		if(MessageBox(_CS(_T("PlayBack.OKCancel")),"ClientDemo2",
			MB_ICONEXCLAMATION | MB_OKCANCEL) == IDOK)
		{
			int Ret = H264_PLAY_ConvertFile((const char*)m_strSrcpath.GetBuffer(0),
				(const char*)m_strDesPath.GetBuffer(0),MEDIA_FILE_AVI,NULL,NULL);
			if ( Ret > 0)
			{
	...
	m_OldDes = m_strDesPath.Mid(m_strDesPath.ReverseFind('\\')+1);
	//CDialog::OnOK();
}
...
void Cdlgcoverfile::OnEnChangeEditDespath()
{

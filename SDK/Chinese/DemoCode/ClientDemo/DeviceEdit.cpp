...
void CDeviceEdit::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_COMBO_LOGINTYPE, m_typeCombo);
...
//{{AFX_MSG_MAP(CDeviceEdit)
	...
	ON_CBN_SELCHANGE(IDC_COMBO_LOGINTYPE, OnSelchangeComboLogintype)
	//}}AFX_MSG_MAP
...
BOOL CDeviceEdit::WriteXML()
{
	...
	if(xml.Load(GET_MODULE_FILE_INFO.strPath + "UserInfo.xml"))
	{
		...
		if (m_devInfo.szIpaddress!=xml.GetChildData())   // 不同的IP地址 写用户名和密码//
		{
...
void CDeviceEdit::OnCHECKSerialID() 
{
	...
	if(m_bSerialID)
	{
		...
		GetDlgItem(IDC_COMBO_LOGINTYPE)->EnableWindow(FALSE);
	}
	else
	{
		GetDlgItem(IDC_EDIT_HOSTPORT)->EnableWindow(FALSE);
		...
		GetDlgItem(IDC_COMBO_LOGINTYPE)->EnableWindow(TRUE);
	}
	UpdateData(FALSE);
	
}

void CDeviceEdit::OnSelchangeComboLogintype() 
{
	// TODO: Add your control notification handler code here	
}
